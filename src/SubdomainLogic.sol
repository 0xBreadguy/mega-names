...
contract SubdomainLogic {
    ...
    function setTokenGate(
        ...
        tokenGates[parentId] = TokenGate({token: token, minBalance: minBalance});
        ...
        if (mode == ISubdomainRouter.Mode.ALLOWLIST) {
            ...
            if (gate.token != address(0) && gate.minBalance > 0) {
                ...
                (bool success, bytes memory data) =
                    gate.token.staticcall(abi.encodeWithSignature("balanceOf(address)", buyer));
