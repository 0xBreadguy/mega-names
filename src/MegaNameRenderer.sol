...
contract MegaNameRenderer is Ownable {
    ...
    function _svg(
        ...
        string memory part3 = string.concat(
            _svgName(topDisplay, bottomDisplay, isSub),
            _svgInfo(labelLen, expiresAt, isSub),
            _svgClose()
        );
    ...
    function _svgTierIcon(
        ...
        if (tier == 1) {
            return string.concat(
                '<polygon points="360,28 370,42 360,56 350,42" fill="none" stroke="',
                ac,
                '" stroke-width="1.5"/>' '<polygon points="360,34 364,42 360,50 356,42" fill="',
                al,
                '"/>'
            );
        }
        if (tier == 2) {
            return string.concat(
                '<polygon points="352,28 358,42 352,56 346,42" fill="none" stroke="',
                ac,
                '" stroke-width="1.2"/>'
                '<polygon points="368,28 374,42 368,56 362,42" fill="none" stroke="',
                ac,
                '" stroke-width="1.2"/>'
            );
        }
        if (tier == 3) {
            return string.concat(
                '<polygon points="360,28 372,35 372,49 360,56 348,49 348,35" fill="none" stroke="',
                ac,
                '" stroke-width="1.2"/>' '<circle cx="360" cy="42" r="6" fill="',
                al,
                '"/>'
            );
        }
        if (tier == 4) {
            return string.concat(
                '<polygon points="360,28 374,39 369,55 351,55 346,39" fill="none" stroke="',
                ac,
                '" stroke-width="1"/>' '<circle cx="360" cy="42" r="4" fill="',
                al,
                '"/>'
            );
        }
        return string.concat(
            '<circle cx="360" cy="42" r="12" fill="none" stroke="',
            ac,
            '" stroke-width="0.8"/>' '<circle cx="360" cy="42" r="3" fill="',
            ac,
            '"/>'
        );
    }
    ...
    function _svgName(
        ...
        if (isSub) {
            ...
            return string.concat(
                ...
                MegaNamesSVG.escapeXML(topDisplay),
                "</text>",
                '<text x="28" y="220" font-family="Helvetica Neue,Arial,sans-serif" font-size="32" fill="rgba(25,25,26,0.40)">',
                MegaNamesSVG.escapeXML(bottomDisplay),
                "</text>"
            );
        }
        ...
        if (len <= 6) fs = "72";
        yp = "196";
        else if (len <= 10) fs = "48";
        yp = "190";
        else if (len <= 15) fs = "34";
        yp = "186";
        else fs = "26";
        yp = "184";
        return string.concat(
            ...
            fs,
            '" text-anchor="middle" fill="#1A1A1A" letter-spacing="1">',
            MegaNamesSVG.escapeXML(topDisplay),
            "</text>"
        );
    }
    ...
    function _svgInfo(
        ...
        string memory right =
            (!isSub && expiresAt > 0) ? string.concat("EXP ", _fmtDate(expiresAt)) : "";
        ...
        return string.concat(
            ...
            '<text x="28" y="359" font-family="monospace" font-size="9" fill="rgba(25,25,26,0.40)">',
            left,
            "</text>"
            '<text x="200" y="359" font-family="monospace" font-size="9" text-anchor="middle" fill="rgba(25,25,26,0.35)" letter-spacing="2">',
            center,
            "</text>"
            '<text x="372" y="359" font-family="monospace" font-size="9" text-anchor="end" fill="rgba(25,25,26,0.35)">',
            right,
            "</text>"
        );
    }
    ...
    function _fmtDate(
        ...
        if (ts == 0) return "---";
        uint256 z = uint256(ts) / 86_400 + 719_468;
